...
module.exports = (payload) => {
  ...
  const createEmitter = ({ colorA, colorB }) => {
    ...
    if (isLoading == 1){
      ...
      let loadSystem = async ()=> {
        ...
        renderer.onParticleCreated = function(p) {
          ...
          const scale = p.scale;
          quat.fromEuler(p.target.qrot, p.rotation.x, p.rotation.y, p.rotation.z);
        ...
        renderer.onParticleUpdate = function(p) {
          ...
          quat.fromEuler(p.target.qrot, dg(p.rotation.x), dg(p.rotation.y), dg(p.rotation.z));
        ...
        system.addEmitter(emitterA).addEmitter(emitterB).addRenderer(renderer);
